...
PRE_SCRIPT = os.path.join(BASE, "01_preprocessing.py")
ML_SCRIPT  = os.path.join(BASE, "03_ml_models.py")
...
def run_setup():
    ...
    if not os.path.exists(RAW):
        st.error(
            ...
            f"Expected at: `{RAW}`\n\n"
            "Please make sure `Nassau_Candy_Distributor.csv` is in the `data/` folder of your repo."
        )
        st.stop()
...
with st.sidebar:
    ...
    lt_thresh = st.slider("Lead Time Threshold (days)",
...
st.markdown(f"**{df.shape[0]:,} orders** | {df['Route'].nunique()} routes | "
...
# ── Tabs ──────────────────────────────────────────────────────────────────────
tabs = st.tabs(["📊 Overview", "🗺️ Geo Map", "🚚 Ship Mode", "🔬 Route Drill-Down",
                "🤖 ML Insights", "🔮 Predictor"])
...
with tabs[1]:
    ...
    state_grp["lat"] = state_grp["State/Province"].map(lambda s: US_STATE_COORDS.get(s, (None, None))[0])
    state_grp["lon"] = state_grp["State/Province"].map(lambda s: US_STATE_COORDS.get(s, (None, None))[1])
    ...
    st.dataframe(region_tbl.style.background_gradient(subset=["Avg_Lead"], cmap="RdYlGn_r"),
...
with tabs[2]:
    ...
    with col1:
        ...
        fig.update_layout(showlegend=False, plot_bgcolor="rgba(0,0,0,0)", title_font_color=BRAND)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col4:
        fig = px.scatter(sm_grp, x="Avg_Lead", y="Efficiency",
                         ...
                         labels={"Avg_Lead": "Avg Lead Time", "Efficiency": "Efficiency Score"})
        fig.update_layout(plot_bgcolor="rgba(0,0,0,0)", title_font_color=BRAND)
        st.plotly_chart(fig, use_container_width=True)

    st.subheader("Ship Mode Summary Table")
    st.dataframe(sm_grp.round(3).style.background_gradient(subset=["Avg_Lead"], cmap="RdYlGn_r"),
                 use_container_width=True)
...
with tabs[3]:
    ...
    selected_route = st.selectbox("Select a Route", sorted(df["Route"].unique()))
    ...
    with st.expander("📋 View raw orders for this route"):
        st.dataframe(
            route_df[["Order ID", "Order Date", "Ship Date", "Ship Mode",
                       "Product Name", "Lead Time", "Is Delayed", "Sales", "Gross Profit"]]
        ...
        with ml_tab1:
            ...
            if "reg" in models:
                ...
                rows = [{"Model": k, "MAE": v["MAE"], "RMSE": v["RMSE"],
                         "R²": v["R2"], "CV R²": v["CV_R2"]} for k, v in res.items()]
...
with tabs[5]:
    ...
    st.markdown("Use the trained ML models to predict shipping lead time and delay risk for a new order.")
...
def load_data():
    if not os.path.exists(DATA):
        st.error("nassau_clean.csv not found. Run 01_preprocessing.py first.")
        st.stop()
    df = pd.read_csv(DATA, parse_dates=["Order Date","Ship Date"])
    return df
...
with tabs[4]:
    ...
    if not models:
        st.warning("⚠️ No trained models found. Run `03_ml_models.py` first.")
    else:
        ml_tab1, ml_tab2, ml_tab3 = st.tabs(["Regression","Classification","Clustering"])
        ...
        with ml_tab1:
            ...
            if "reg" in models:
                ...
                st.dataframe(pd.DataFrame(rows).set_index("Model").round(4),
                             use_container_width=True)
                if os.path.exists(os.path.join(BASE,"../outputs/ml/reg_feature_importance.png")):
                    st.image(os.path.join(BASE,"../outputs/ml/reg_feature_importance.png"))
                if os.path.exists(os.path.join(BASE,"../outputs/ml/reg_actual_vs_predicted.png")):
                    st.image(os.path.join(BASE,"../outputs/ml/reg_actual_vs_predicted.png"))

        with ml_tab2:
            st.markdown("### Delay Classification")
            if "clf" in models:
                if os.path.exists(os.path.join(BASE,"../outputs/ml/clf_confusion_matrix.png")):
                    st.image(os.path.join(BASE,"../outputs/ml/clf_confusion_matrix.png"))
                if os.path.exists(os.path.join(BASE,"../outputs/ml/clf_feature_importance.png")):
                    st.image(os.path.join(BASE,"../outputs/ml/clf_feature_importance.png"))
        ...
        with ml_tab3:
            ...
            if "clu" in models:
                ...
                st.dataframe(route_df_clu[["Route","Cluster Label","Avg_Lead_Time",
                                           "Delay_Rate","Total_Orders","Avg_Efficiency"]]
                             .sort_values("Avg_Lead_Time").round(3),
                             use_container_width=True)
...
with tabs[5]:
    ...
    if not models:
        st.warning("⚠️ Run `03_ml_models.py` first to enable predictions.")
    else:
        with st.form("predict_form"):
            col1, col2, col3 = st.columns(3)
            with col1:
        ...
        if submitted:
            input_raw = {
                "Ship Mode": p_ship_mode, "Factory": p_factory, "Region": p_region,
                "Division": p_division, "Sales": p_sales, "Units": p_units,
                "Gross Profit": p_profit, "Cost": p_cost,
                "Order Month": p_month, "Order Quarter": (p_month-1)//3+1,
                "Order DayOfWeek": 1,
            }
            from sklearn.preprocessing import LabelEncoder
            NUMERIC_F = ["Sales","Units","Gross Profit","Cost","Order Month","Order Quarter","Order DayOfWeek"]
            CATEG_F   = ["Ship Mode","Factory","Region","Division"]
            ...
            if "reg" in models:
                ...
                X_pred = []
                for f in NUMERIC_F:
                    X_pred.append(float(input_raw[f]))
                for f in CATEG_F:
                    le = le_dict[f]
                    val= input_raw[f]
                    try:
                        ...
                        enc = 0
                    X_pred.append(float(enc))
                X_arr = np.array(X_pred).reshape(1, -1)
                pred_lt = reg["model"].predict(X_arr)[0]
                st.success(f"⏱ **Predicted Lead Time:** `{pred_lt:.1f} days`")
            ...
            if "clf" in models:
                ...
                for f in CATEG_F:
                    ...
                    X_pred.append(float(enc))
                X_arr  = np.array(X_pred).reshape(1, -1)
                pred_cl= clf["model"].predict(X_arr)[0]
                pred_pr= clf["model"].predict_proba(X_arr)[0]
                label  = "🔴 **DELAYED**" if pred_cl == 1 else "🟢 **EFFICIENT**"
                conf   = max(pred_pr) * 100
                st.info(f"🚦 **Delay Prediction:** {label}  (confidence: {conf:.1f}%)")
